...
export function QuantityPricing({ productId, basePrice, quantity }: QuantityPricingProps) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-medium text-muted-foreground">Quantity Discounts</p>
      <div className="flex flex-wrap gap-1.5">
        {tiers.map((t) => {
          ...
          return (
            ...
            >
              {t.min_quantity}+ → {t.discount_percent}% off
            </Badge>
          );
        })}
      </div>
      {hasDiscount && (
        <p className="text-sm">
          <span className="text-muted-foreground line-through mr-2">{basePrice.toLocaleString()} DA</span>
          <span className="font-bold text-accent">{Math.round(currentPrice).toLocaleString()} DA/unit</span>
        </p>
      )}
    </div>
  );
